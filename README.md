...
`install` 명령은 `python`, `python-cryptography`, `ffmpeg`, 컴파일 도구를 확인해 누락된 Termux 패키지만 설치합니다. `.venv`가 없으면 `--system-site-packages` 방식으로 생성하고 `requirements.txt`도 자동 설치합니다. MP4 변환에는 `ffmpeg`가 반드시 필요합니다.
...
`start`와 `restart`도 서버를 실행하기 전에 같은 패키지 및 Python 의존성 검사를 수행하고 누락된 항목을 자동 설치합니다.
